...
class HelloArRenderer(val activity: HelloArActivity) :
  ...
  override fun onSurfaceCreated(render: SampleRender) {
    ...
    footLocation = setFootLocation(5.232413 ,52.358581, 3.0) //LOCATIE FLORIADE
    //footLocation = setFootLocation(4.471314122006459 ,52.16873237045897, 3.0) //LOCATIE PARK LEIDEN
    ...
    try {
      ...
      virtualObjectMesh = Mesh.createFromAsset(render, "models/leg.obj")
      virtualObjectShader =
        Shader.createFromAssets(
          render,
  ...
  private fun handleTap(frame: Frame, camera: Camera) {
    ...
    val hitResultList =
      if (activity.instantPlacementSettings.isInstantPlacementEnabled) {
        ...
        //frame.hitTestInstantPlacement(differenceLongitude.toFloat(), differenceLatitude.toFloat(), differenceVector.toFloat())
        //frame.hitTestInstantPlacement(500f, 1100f, APPROXIMATE_DISTANCE_METERS)
      } else {
        frame.hitTest(tap)
      }
  ...
  private fun getLastKnownLocation(){ //private fun getLastKnownLocation(): Location {
    ...
    fusedLocationClient.lastLocation
      .addOnSuccessListener { location->
        ...
        println("PrintInfo differenceVector = " + differenceVector.toString())
        println("PrintInfo Bearing= " + bearingToFoot.toString())
      }
  }

  private fun calculateDistance(location: Location, currentLocation: Location): Float {
    val distanceVector = location.distanceTo(currentLocation)
  ...
  private fun calculateBearing(location: Location, currentLocation: Location): Float {
    val bearing = currentLocation.bearingTo(location)
...
//514, 167
//515 werkt als circle om je heen zogezegd (vast afstand)